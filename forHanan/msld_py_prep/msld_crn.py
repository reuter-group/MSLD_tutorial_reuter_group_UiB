...
def MsldCRN(mcsout,outdir,inFrag,inCore,ChkQChange=True,verbose=False,debug=False,ll=True):
    ...
    for mol in range(len(mols)):
        ...
        if float(rtfinfo[mol]['QNET']) != float(qnet):
            print("We have a charge change between the reflig ("+mols[refnum]+' and '+mols[mol]+').',\
                  "ref_qnet = "+qnet,"; mol_qnet = "+rtfinfo[mol]['QNET'])
            if not ChkQChange:
    ...
    tsum=float(str(np.around(QQsum+sitesum,decimals=dec)))  # total charge of msld ligand
    ...
    sitechk=float(str(np.around(sitesum-qnet,decimals=dec)))
